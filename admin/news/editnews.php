...
<section class="home-section">
    ...
    <form method="post"enctype="multipart/form-data">
        ...
        <div>
            <img src="../imageupload/news/<?php echo $result['news_image'];?>" width="120px" height="100px"><br>
            <label>ID:<?php echo $result["news_ID"]?></label>
        </div>
        <div>
            <label>ชื่อสินค้า :</label>
            <input type="text" value="<?php echo $result["news_name"]?>" name="newsname" required>
        </div>
        <div>
            <label>รายละเอียด :</label>
            <input type="text" value="<?php echo $result["news_details"]?>" name="newsdetails"  required>
        </div>
        <div>
            <label>รูปภาพ :</label>
    ...
    if(isset($_POST["Submit"])){
        ...
        $imagenews = $_FILES["newsimage"]["name"];
        $updatesql = "UPDATE news SET news_name='$name',news_details = '$details',news_image = '$imagenews' WHERE news_ID = $ID";
        $query = mysqli_query($conn,$updatesql);
        $target_file = '../imageupload/news/' . basename($imagenews);
        if($query){
