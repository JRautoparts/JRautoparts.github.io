...
<section class="home-section">
    ...
    <form method="post"enctype="multipart/form-data">
        ...
        <div>
            <img src="../imageupload/promotion/<?php echo $result['promotion_image'];?>" width="120px" height="100px"><br>
            <label>ID:<?php echo $result["promotion_ID"]?></label>
        </div>
        <div>
            <label>ชื่อสินค้า :</label>
            <input type="text" value="<?php echo $result["promotion_name"]?>" name="promotionname" required>
        </div>
        <div>
            <label>รายละเอียด :</label>
            <input type="text" value="<?php echo $result["promotion_details"]?>" name="promotiondetails"  required>
        </div>
        <div>
            <label>รูปภาพ :</label>
        ...
        <button><a href="./promotioninformation.php">ย้อนกลับ</a></button>
    </form>
    <?php
    if(isset($_POST["Submit"])){
        $name = $_POST["promotionname"];
        $details = $_POST["promotiondetails"];
        $imagepromotion = $_FILES["promotionimage"]["name"];
        $updatesql = "UPDATE promotion SET promotion_name='$name',promotion_details = '$details',promotion_image = '$imagepromotion' WHERE promotion_ID = $ID";
        $query = mysqli_query($conn,$updatesql);
        $target_file = '../imageupload/promotion/' . basename($imagepromotion);
        if($query){
            move_uploaded_file($_FILES['promotionimage']['tmp_name'],"$target_file");
            header("Location: ./editpromotion.php?ID=$ID");
