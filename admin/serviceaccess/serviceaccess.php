...
if (!isset($_SESSION["username"])) {
    ...
    $query = "SELECT * FROM service_access WHERE CONCAT(carlicense_num,status_name) LIKE '%" . $valueToSearch . "%'";
    $search_result = filterTable($query);
} else {
    $query = "SELECT * FROM service_access";
    $search_result = filterTable($query);
}
...
<section class="home-section">
    ...
    <div>
        ...
        ?>
        <button onclick="window.location.href='./addserviceaccess.php'">เพิ่มข้อมูล</button>
        <table id="datatable" class="display" style="width:70%">
            <thead>
            <tr>
                <th>ลำดับการรับบริการ</th>
                <th>เลขทะเบียนรถ</th>
                <th>สถานะ</th>
                <th>รายละเอียด</th>
                <th>วันที่</th>
            ...
            <?php while ($row = mysqli_fetch_array($search_result)) : ?>
                <tr>
                    <th><?php echo $row["service_access_ID"] ?></th>
                    <th><?php echo $row["carlicense_num"] ?></th>
                    <th><?php echo  $row["status_name"] ?></th>
                    <th><?php echo $row["service_access_details"] ?></th>
                    <th><?php if ($row["service_access_edit"] == 0) {
                            echo "<p>เพิ่ม</p>";
                        } else {
                            echo "<p>แก้ไข</p>";
                        } ?><?php echo $row["service_access_date"] ?></th>
                    <th><?php if ($row["service_access_edit"] == 0) {
                            echo "<p>เพิ่ม</p>";
                        } else {
                            echo "<p>แก้ไข</p>";
                        } ?><?php echo $row["service_access_time"] ?></th>
                    <th><?php if ($row["service_access_edit"] == 0) {
                            echo "<p>เพิ่มโดย</p>";
                        } else {
                            echo "<p>แก้ไขโดย</p>";
                        ...
                        endwhile;
                        ?></th>
                    <th><a href="./deleteserviceaccess.php?ID=<?php echo $row["service_access_ID"] ?>">ลบ</a></th>
                    <th><a href="./editserviceaccess.php?ID=<?php echo $row["service_access_ID"] ?>">แก้ไข</a></th>
                </tr>
            <?php endwhile; ?>
            </tbody>
        </table>
