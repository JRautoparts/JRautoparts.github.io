...
ob_start();
include('../../connect.php');
$sqlcustomerservicejoinservice_accesscount = "SELECT COUNT(customer_services.carlicense_num) AS count FROM customer_services LEFT JOIN service_access ON customer_services.carlicense_num = service_access.carlicense_num WHERE service_access.carlicense_num IS NULL";
$resultcustomerservicejoinservice_accesscount = mysqli_query($conn, $sqlcustomerservicejoinservice_accesscount);
$data=mysqli_fetch_assoc($resultcustomerservicejoinservice_accesscount);
if(!isset($_SESSION["username"])){
    ...
    $_SESSION["success"] = "ยังไม่มีเลขทะเบียนรถเพิ่มเข้ามา";
}
...
<section class="home-section">
    <body>
    <h1>เพิ่มข้อมูลการเข้ารับบริการ</h1>
    <form method="post">
        <div>
        ...
        <div>
            <input type="text" name="service_access_details" placeholder="ข้อมูลรายละเอียด">
        </div>
        <div>
    ...
    if (isset($_POST["submit"])){
        ...
        $date =  date("Y-m-d");
        $time =  date(" H:i:s");
        $sqlservice_access = "INSERT INTO service_access (carlicense_num,status_name,service_access_details,service_access_date,service_access_time,service_access_edit,admin_ID) VALUES ('$carlicense','$status_name','$detail','$date','$time',0,'$idadmin')";
        $query = mysqli_query($conn,$sqlservice_access);
        if ($query){
            header("Location: ./serviceaccess.php");
