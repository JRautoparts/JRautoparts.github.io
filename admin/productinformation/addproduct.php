...
$sqlproducttype = "SELECT product_type_ID,product_type_name FROM product_type ";
...
<section class="home-section">
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div>
            ...
            <select name ="producttype">
                <?php while($result = mysqli_fetch_array($resultproducttype)){
                    echo "<option value = '".$result['product_type_ID']."'>".$result['product_type_name']."</option>" ;
                }
                ?>
            </select>
        </div>
        <div>
            ราคา :
            <input type="text" name="productprice" pattern="[0-9]{}">
        </div>
        <div>
            รูปภาพ :
    ...
    if(isset($_POST["submit"])){
        ...
        // print_r($_POST);
        $target_file = '../imageupload/product/' . basename($productimg);
        $sql = "INSERT INTO product (product_name,product_type_ID,product_image,product_price,admin_ID) VALUES ('$name',$producttype,'$productimg','$productprice','$idadmin')";
