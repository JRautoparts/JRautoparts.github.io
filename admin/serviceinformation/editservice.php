...
<section class="home-section">
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <div>
            <label>ID:<?php echo $result["service_ID"]?></label>
        </div>
        <div>
            <input type="text" value="<?php echo $result["service_name"]?>" name="servicename" required>
        </div>
        <div>
        ...
        <button><a href="./serviceinformation.php">ย้อนกลับ</a></button>
    </form>
    <?php
    if(isset($_POST["Submit"])){
        $name = $_POST["servicename"];
        $imageservice = $_FILES["serviceimg"]["name"];
        $imagesql = $result["service_image"];
        $target_file = '../imageupload/service/' . basename($imageservice);

        if(empty($imageservice)){
            $updatesql = "UPDATE service SET service_name='$name',service_image = '$imagesql' WHERE service_ID = '$ID' ";
            $query = mysqli_query($conn,$updatesql);
            if($query){
                header("Location: ./serviceinformation.php?ID=$ID");
            ...
            $updatesql = "UPDATE service SET service_name='$name',service_image = '$imageservice' WHERE service_ID = '$ID' ";
            $query = mysqli_query($conn,$updatesql);
            if($query){
                move_uploaded_file($_FILES['serviceimg']['tmp_name'],"$target_file");
                header("Location: ./serviceinformation.php");
                $_SESSION["success"] = "แก้ไขข้อมูลเรียบร้อย";
            }
            else{
