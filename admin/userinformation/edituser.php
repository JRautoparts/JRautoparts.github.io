...
<section class="home-section">
    ...
    <form method="post">
        <h2>ข้อมูลบริการลูกค้า</h2>
        <h2>Admin:<?php echo $_SESSION["username"];?></h2>
        <div>
            <label>ID:<?php echo $result["customer_ID"]?></label>
        </div>
        <div>
            <input type="text" value="<?php echo $result["customer_name"]?>" name="nameuser" required>
        </div>
        <div>
            <input type="tel" value="<?php echo $result["customer_tel"]?>" id="phone" name="phone" pattern="[0-9]{10}" maxlength="10" required>
        </div>
        <div>
            <input type="text" value="<?php echo $result["carlicense_num"]?>" name="carlicense" required>
        </div>
    ...
    if(isset($_POST["Submit"])){
        ...
        $updatesql = "UPDATE customer SET customer_name='$name',customer_tel = '$phone',carlicense_num = '$carlicense' WHERE customer_ID = $ID";
